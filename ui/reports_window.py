"""
Окно отчётов и экспорта.
Поддерживает мультивыбор студентов — генерация документов для нескольких сразу.
"""
...
class ReportsWindow(tk.Toplevel):
    def __init__(self, parent, user: dict, selected_student_ids=None):
        ...
        self.user = user
        # поддержка и старого (int) и нового (list) вызова
    ...
    def _build(self):
        tk.Label(self, text="Отчёты и экспорт в Word",
                 ...
                 bg="#1a3a5c", fg="white", padx=12, pady=8).pack(fill="x")

        main_frame = tk.Frame(self)
        main_frame.pack(fill="both", expand=True, padx=10, pady=10)

        # ── левая: список студентов ───────────────────────────────────────────
        left = ttk.LabelFrame(main_frame, text="Выбор студентов (Ctrl+клик — несколько)", padding=6)
        left.pack(side="left", fill="both", expand=True)

        sf = tk.Frame(left); sf.pack(fill="x", pady=(0, 4))
        ...
        # кнопки выбора
        bf = tk.Frame(left); bf.pack(fill="x", pady=(0, 4))
        tk.Button(bf, text="Выбрать всех", bg="#546e7a", fg="white",
                  font=("Segoe UI", 8), relief="flat", cursor="hand2",
                  command=self._select_all_students).pack(side="left", padx=(0, 4))
        tk.Button(bf, text="Снять выбор", bg="#546e7a", fg="white",
                  font=("Segoe UI", 8), relief="flat", cursor="hand2",
                  command=self._deselect_all_students).pack(side="left")
        self._sel_count_lbl = tk.Label(bf, text="", font=("Segoe UI", 8),
                                       fg="#2980b9"); self._sel_count_lbl.pack(side="right")

        lf = tk.Frame(left); lf.pack(fill="both", expand=True)
        self._slist = tk.Listbox(lf, font=FONT, selectmode="extended",
        ...
        self._selected_lbl = tk.Label(right, text="Студенты не выбраны",
                                      font=("Segoe UI", 9, "italic"),
                                      fg="#555", wraplength=210)
        ...
        for lbl, color, cmd in [
            ...
            tk.Button(right, text=lbl, bg=color, fg="white",
                      font=FONT, relief="flat", cursor="hand2",
                      width=24, pady=5, command=cmd).pack(pady=2)
    ...
    def _get_selected_students(self) -> list:
        """Вернуть list[dict] выбранных студентов."""
        return [self._students[i] for i in self._slist.curselection()]

    def _require_selection(self) -> bool:
        if not self._slist.curselection():
            messagebox.showinfo("Выбор", "Выберите хотя бы одного студента слева",
                                parent=self)
            return False
        return True
    ...
    def _get_country_name(student) -> str:
        if not student:
            return ""
        cid = student.get("country_id") if isinstance(student, dict) else student["country_id"]
        if not cid:
            return ""
    ...
    # ── Word: генерация для нескольких студентов ──────────────────────────────

    def _gen_spravka(self):
        if not self._require_selection():
            return
        self._gen_word_for_selected("spravka")

    def _gen_visa_anketa(self):
        if not self._require_selection():
            return
        self._gen_word_for_selected("visa")

    def _gen_word_for_selected(self, doc_type: str):
        try:
            from word_templates import generate_spravka, generate_visa_anketa, check_docx_available
            ...
            for s_info in selected:
                student = db.get_student_by_id(s_info["id"])
                country = self._get_country_name(dict(student))
                try:
                    if doc_type == "spravka":
                        p = generate_spravka(dict(student), country)
                    else:
                        p = generate_visa_anketa(dict(student), country)
                    if p:
                        paths.append(p)
                except Exception as e:
                    errors.append(f"{s_info['full_name_ru']}: {e}")
    ...
    def _report_visas(self):
        ...
        for delta, r, visa_to in warn:
            ...
            lines.append(f"{r['full_name_ru']:<35} {r['citizenship']:<18} "
    ...
    def _report_audit(self):
        if self.user["role"] not in ("admin", "operator"):
            messagebox.showinfo("Доступ", "Только для администраторов/операторов", parent=self)
            return
        ...
        lines = [
            ...
            "=" * 90,
            f"{'Время':<20} {'Пользователь':<18} {'Действие':<10} {'Таблица':<12} {'Подробности':<30}",
            "-" * 90,
        ]
        for r in rows:
            lines.append(
